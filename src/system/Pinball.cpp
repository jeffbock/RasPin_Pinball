...
// Helper: returns true if the given path is an existing directory (cross-platform)
...
// Walks up from the executable location to find the project root (identified by
// the presence of a "src/user" subdirectory) and changes CWD to it.
// This makes all resource paths work correctly regardless of where the EXE lives
// in the build tree (e.g. build/windows/debug/, build/raspi/release/, etc.).
...
bool AdjustWorkingDirectory(const char* exePath) {
    ...
    // Resolve exe directory to an absolute path
    ...
    // Walk up from the exe directory, searching for the project root.
    // The project root is identified by having a "src/user" subdirectory.
    // We allow up to 6 levels of nesting to handle deep build trees
    // (e.g. build/windows/debug/ is 3 levels below the project root).
    std::string searchDir = absExeDir;
    std::string projectRoot;
    for (int level = 0; level <= 6; level++) {
        if (PathIsDirectory(searchDir + "/src/user")) {
    ...
    if (!projectRoot.empty()) {
        if (std::string(cwd) != projectRoot) {
            if (chdir(projectRoot.c_str()) != 0) {
                std::cerr << "ERROR: Failed to change working directory to project root: " << strerror(errno) << std::endl;
                return false;
            }
        }
    } else {
        std::cerr << "WARNING: Could not locate project root (src/user not found within 6 levels of exe). Using CWD as-is." << std::endl;
    }

    // Report the final working directory
