...
bool PBEngine::pbeLoadDefaultBackground(){
    ...
    m_BootUpConsoleId = gfxLoadSprite("Console", "src/user/resources/textures/Console.png", GFX_PNG, GFX_NOMAP, GFX_UPPERLEFT, false, true);
    gfxSetColor(m_BootUpConsoleId, 255, 255, 255, 196);
    gfxSetScaleFactor(m_BootUpConsoleId, 1.05, false);

    m_BootUpStarsId = gfxLoadSprite("Stars", "src/user/resources/textures/stars.png", GFX_PNG, GFX_NOMAP, GFX_CENTER, false, true);
...
bool PBEngine::pbeRenderOverlay(unsigned long currentTick, unsigned long lastTick){
    ...
    gfxRenderShadowString(m_defaultFontSpriteId, stateDisplay1, (PB_SCREENWIDTH / 2), 5, 0.4, GFX_TEXTCENTER, 0, 0, 0, 255, 1);
    ...
    gfxSetColor(m_defaultFontSpriteId, 255, 255, 0, 255);  // Yellow for state display
    gfxRenderShadowString(m_defaultFontSpriteId, stateDisplay2, (PB_SCREENWIDTH / 2), 26, 0.4, GFX_TEXTCENTER, 0, 0, 0, 255, 1);
    ...
    for (int i = 0; i < NUM_INPUTS; i++) {
        #ifndef ENABLE_PINBALL_HARDWARE
        std::string temp = g_inputDef[i].inputName + " (" + g_inputDef[i].simMapKey + "): ";
        #else
        std::string temp = g_inputDef[i].inputName + ": ";
        #endif
        
        int x = 20;                   // Single column
        int y = 66 + (i * 25);        // Start below header with proper spacing (adjusted for second state line)
...
bool PBEngine::pbeLoadTestSandbox(){
    ...
    if (!m_simpleFlipEjector) {
        m_simpleFlipEjector = new pbdHopperEjector(this, IDI_BALLREADY, IDO_EJECT, IDI_BALLDELIVERED);
        pbeAddDevice(m_simpleFlipEjector);
    }
    ...
    if (!m_sandboxVideoPlayer) {
        ...
        m_sandboxVideoSpriteId = m_sandboxVideoPlayer->pbvpLoadVideo(
            "src/user/resources/videos/darktown_sound_h264.mp4",
            videoX,
            videoY,
    ...
    if (!m_sandboxDiceLoaded) {
        m_sandboxD20ModelId = pb3dLoadModel("src/user/resources/3d/diceset.glb");
    ...
    // Load crystalwing animated 3D model
    // forceStatic=true: load all meshes via the simple 8-float static path (no skinning)
    // to isolate geometry rendering from any skinned-shader issues.
    if (!m_sandboxWingLoaded) {
        m_sandboxWingModelId = pb3dLoadModel("src/user/resources/3d/crystalwing.glb");
...
bool PBEngine::pbeRenderTestSandbox(unsigned long currentTick, unsigned long lastTick){
    ...
    if (m_RestartTestSandbox) {
        ...
        if (m_sandboxWingLoaded) {
            ...
            pb3dDestroyInstance(m_sandboxWingInstance);
            pb3dUnloadModel(m_sandboxWingModelId);
            m_sandboxWingInstance = 0;
            m_sandboxWingModelId = 0;
            m_sandboxWingLoaded = false;
        }
    ...
    // Update all 3D animations once per frame (dice transforms + wing skeleton)
...
bool PBEngine::pbeRenderSimpleFlipMode(unsigned long currentTick, unsigned long lastTick){
    ...
    if (m_RestartSimpleFlipMode) {
        ...
        // Enable auto outputs (flippers/slings)
        SetAutoOutputEnable(true);

        // Turn on all gameplay indicator lights
        SendOutputMsg(PB_OMSG_GENERIC_IO, IDO_STARTLED,   PB_ON, false);
        SendOutputMsg(PB_OMSG_LED,        IDO_LSLINGLED,  PB_ON, false);
        SendOutputMsg(PB_OMSG_LED,        IDO_RSLINGLED,  PB_ON, false);
        ...
        // Set NeoPixel strip to white
        SendNeoPixelAllMsg(IDO_NEOPIXEL0, PB_LEDWHITE, 255);
    ...
    switch (m_mainState) {
        case PB_BOOTUP: {
            ...
            if (inputMessage.inputMsg == PB_IMSG_BUTTON && inputMessage.inputState == PB_ON) {
                ...
                else if (inputMessage.inputId == IDI_LFLIP || inputMessage.inputId == IDI_LACTIVATE) {
                    if (m_consoleStartLine > 0) {
                        m_consoleStartLine--;
                    }
                }
                // Right flipper or right activate: scroll console one line later (down)
                else if (inputMessage.inputId == IDI_RFLIP || inputMessage.inputId == IDI_RACTIVATE) {
        ...
        case PB_STARTMENU: {
            ...
            if (inputMessage.inputMsg == PB_IMSG_BUTTON && inputMessage.inputState == PB_ON) {
                ...
                if (inputMessage.inputId == IDI_RFLIP) {
                ...
                if ((inputMessage.inputId == IDI_RACTIVATE) || (inputMessage.inputId == IDI_LACTIVATE)) {
        ...
        case PB_DIAGNOSTICS: {
            ...
            if (((inputMessage.inputId == IDI_RACTIVATE) || (inputMessage.inputId == IDI_LACTIVATE)) && inputMessage.inputState == PB_ON){
                switch (m_CurrentDiagnosticsItem) {
                    ...
                    case (2): if ((inputMessage.inputId == IDI_RACTIVATE) || (inputMessage.inputId == IDI_LACTIVATE)) {
                        if (m_EnableOverlay) m_EnableOverlay = false;
                        else m_EnableOverlay = true;
                        g_PBEngine.m_soundSystem.pbsPlayEffect(SOUNDCLICK);
                    }
                    break;
                    case (3): if ((inputMessage.inputId == IDI_RACTIVATE) || (inputMessage.inputId == IDI_LACTIVATE)) {
                        if (m_ShowFPS) m_ShowFPS = false;
                        else m_ShowFPS = true;
                        g_PBEngine.m_soundSystem.pbsPlayEffect(SOUNDCLICK);
                    }
                    break;
                    case (4): if ((inputMessage.inputId == IDI_RACTIVATE) || (inputMessage.inputId == IDI_LACTIVATE)) {
                        m_mainState = PB_BOOTUP;
                        m_RestartBootUp = true;
                        g_PBEngine.m_soundSystem.pbsPlayEffect(SOUNDCLICK);
                    }
        ...
        case PB_TESTSANDBOX: {
            ...
            if (inputMessage.inputMsg == PB_IMSG_BUTTON && inputMessage.inputState == PB_ON) {
                ...
                if (inputMessage.inputId == IDI_START) {
                    ...
                    if (m_simpleFlipEjector) {
                        m_simpleFlipEjector->pbdEnable(false);
                        m_simpleFlipEjector->pbdInit();
                        m_simpleFlipEjector = nullptr;  // Will be deleted by pbeClearDevices
                    }
                ...
                if (inputMessage.inputId == IDI_LFLIP) {
                    ...
                    if (testCount % 3 == 0) {  
                        ...
                        SendOutputMsg(PB_OMSG_GENERIC_IO, IDO_LSLING, PB_ON, true);
                        SendOutputMsg(PB_OMSG_GENERIC_IO, IDO_EJECT, PB_ON, true);
                        ...
                        if (m_sandboxNeoPixelStepMode) {
                            ...
                            switch(m_sandboxNeoPixelAnimMode) {
                                ...
                                case 5:
                                    ...
                                    break;
                                case 6:
                                    neoPixelSnake(0, 0, 64, 255, 0, 0, 10, true, IDO_NEOPIXEL0, 100);
                                    break;
                            }
                        } else {
        ...
        case PB_SIMPLEFLIPMODE: {
            ...
            if (inputMessage.inputMsg == PB_IMSG_BUTTON && inputMessage.inputState == PB_ON) {
                if (inputMessage.inputId == IDI_START) {
                    ...
                    // Turn off all gameplay indicator lights
                    SendOutputMsg(PB_OMSG_GENERIC_IO, IDO_STARTLED,   PB_OFF, false);
                    SendOutputMsg(PB_OMSG_LED,        IDO_LSLINGLED,  PB_OFF, false);
                    SendOutputMsg(PB_OMSG_LED,        IDO_RSLINGLED,  PB_OFF, false);
...
bool PBEngine::pbeSetTimer(unsigned int timerId, unsigned int timerValueMS, bool repeat) {
    ...
    m_timerQueue.push_back(timerEntry);
    return true;
}

// Process all timers, check for expired timers,
// and send input messages for any that have expired.
// Uses in-place vector processing to avoid per-frame allocations.
void PBEngine::pbeProcessTimers() {
    ...
    // Process all timers in-place, removing expired ones
    for (size_t i = 0; i < m_timerQueue.size(); ) {
        stTimerEntry& timerEntry = m_timerQueue[i];
        ...
        if (currentTick >= timerEntry.expireTickMS) {
            ...
            // If this is a repeat timer, restart it in-place
            if (timerEntry.repeat) {
                timerEntry.startTickMS = currentTick;
                timerEntry.expireTickMS = currentTick + timerEntry.durationMS;
                ++i;  // Keep this entry, advance
            } else {
                // Remove by swapping with last element (order doesn't matter for timers)
                m_timerQueue[i] = m_timerQueue.back();
                m_timerQueue.pop_back();
                // Don't increment i - need to check the swapped element
            }
        } else {
            ++i;  // Timer still active, advance
        }
    }
}
...
bool PBEngine::pbeTimerActive(unsigned int timerId) {
    ...
    // Search through all timers in the vector
    for (const auto& timerEntry : m_timerQueue) {
...
// Remove a timer with the given ID if it exists
void PBEngine::pbeTimerStop(unsigned int timerId) {
    ...
    // Remove all timers matching the given ID
    m_timerQueue.erase(
        std::remove_if(m_timerQueue.begin(), m_timerQueue.end(),
            [timerId](const stTimerEntry& entry) { return entry.timerId == timerId; }),
        m_timerQueue.end());
}
